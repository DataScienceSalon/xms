...
3. **Degrees of Freedom:** The contingency table must be associated with a chi-square distribution with two or more degrees of freedom.  
...
`r kfigr::figr(label = "trendSummary", prefix = TRUE, link = TRUE, type="Table")`: Summary statistics for proportion of non-traditional opinion
...
For the former  `r kfigr::figr(label = "trendSummary", prefix = TRUE, link = TRUE, type="Table")` reveals that the proportion of non-traditional opinion dropped by `r -round(analysis$PctChange[1], 0)`% from a high of `r round(analysis$Max[1], 2)` in `r round(analysis$YearHigh[1], 0)` to a low of `r round(analysis$Min[1], 2)` in `r round(analysis$YearLow[1], 0)`. The most significant decline of `r -round(analysis$PctChange[2], 0)`%  was among men over this time period from a high of `r round(analysis$Max[2], 2)` in `r round(analysis$YearHigh[2], 0)` to a low of `r round(analysis$Min[2], 2)` in `r round(analysis$YearLow[2], 0)`. The proportion of non-traditional opinion among women fell by  `r -round(analysis$PctChange[3], 2)`%

The trend appeared to reverse over the period from 2012-2016. Overall, the proportion of non-traditional opinion increased by `r round(analysis$PctChange[4], 0)`%. The most significant increase was among women where the proportion of opinion grew  `r round(analysis$PctChange[6], 0)`%  from `r round(analysis$Min[6], 0)` in `r round(analysis$YearLow[6], 0)` to `r round(analysis$Max[6], 0)` in `r round(analysis$YearHigh[6], 0)`.
...
2. **Success/Failure:** The minimum number of successes/failures was `r min(pre2012$tables$freqTbl)`, exceeding the 10-count minimum threshold.
...
The percent change in the proportion of non-traditional opinion overall was `r years$Pct.Change[1]`%, and `r years$Pct.Change[2]`%, and `r years$Pct.Change[3]`% for male and female respondents, respectively.
...
The percent change in the proportion of non-traditional opinion overall was `r years$Pct.Change[4]`%, and `r years$Pct.Change[5]`%, and `r years$Pct.Change[6]`% for male and female respondents, respectively.
...
1. From 1973-2011, the proportion of people holding non-traditional views with respect to extra-marital conduct decreased by `r -round(pre2012$analysis$PctChange[1], 0)`% from a high of `r round(pre2012$analysis$Max[1], 2)` in `r round(pre2012$analysis$YearHigh[1], 0)` to a low of `r round(pre2012$analysis$Min[1], 2)` in `r round(pre2012$analysis$YearLow[1], 0)`.  
2. From 2012-2016 the trend reverses. The proportion of non-traditional opinion increased by `r round(post2012$analysis$PctChange[4], 0)`%. The most significant increase was among women where the proportion of opinion grew  `r round(post2012$analysis$PctChange[6], 0)`%  from `r round(post2012$analysis$Min[6], 0)` in `r round(post2012$analysis$YearLow[6], 0)` to `r round(post2012$analysis$Max[6], 0)` in `r round(post2012$analysis$YearHigh[6], 0)`.  
3. Geograhic region is a significant indicator of opinion w.r.t extra-marital conduct. Nearly `r round(region$analysis$CumPct[7], -1)`% of the non-traditional opinion in the United States from 2012 to 2016 emerged from the Northeast (`r round(region$analysis$PctProp[6], 0)`%) and Pacific regions (`r round(region$analysis$PctProp[7], 0)`%) . Add in the Mountain region (`r round(region$analysis$PctProp[8], 0)`%) , and this percentage increases to `r round(region$analysis$CumPct[8], -1)`%. On average, respondents in the Northeast and Pacific regions were `r analysis$RelativePct[1]`% and `r analysis$RelativePct[2]`% more likely to have non-traditional opinions. The lowest proportion of non-traditional opinion was found in the MidWest and South, each accounting for approximately `r round(mean(region$analysis$PctProp[9], region$analysis$PctProp[10]), 0)`% of the non-traditional opinion in the U.S.  
