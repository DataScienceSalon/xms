...
**Single Proportion**  
This type of hypothesis statement, used in the univariate analysis of categorical variables, was used to estimate the true poulation parameter and as follows:  
$H_0$ $p - \hat{p} = 0$   
$H_a$ $p - \hat{p} \neq 0$    
where:   
...
This type of hypothesis statement was used in the multivarate analysis to determine the true difference in the proportion of non-traditional opinion for two populations and took the following form.  
$H_0$ $p_1 - p_2 = 0$   
$H_a$ $p_1 - p_2 \neq 0$    Two sided, or  
...
The z-statistic was used to calculate a p-value by comparing the value of the statistic for each random variable $X$ to a *standard normal* distribution ($X \sim N(\mu = 0, \sigma^2 = 1)$. The selected level of confidence for all statistical tests was 95% ($\alpha = 0.05$). If the p-value was less than or equal to $\alpha$, the null hypothesis was rejected.  Otherwise, the null hypothesis was not rejected. The p-value for a guassian distribution was obtained using the R statistical software package and is computed as follows:  
...
the 25-44 age group was decidely more likely than other age groups to hold non-traditional opinions w.r.t. extra-marital conduct.
...
Differences in proportion of opinion, as indicated in `r kfigr::figr(label = "maritalDPTest", prefix = TRUE, link = TRUE, type="Table")`, were significant across most comparisoins.  The largest differences were among those who were married or widowed and those who were separated.
...
# Conclusion
Amid the growing amount of research citing a dramatic shift in the marriage institution, one characterized by increasing acceptance of same-sex marriage, open marriages, and consentual non-manogamy (CNM) arrangements, I sought to investigate the degree to which this shift would be evinced by available data. More specifically, I focused on understanding the nature and evolution of a single aspect of marriage: opinions vis-a-vis fidelity. 
...
That said, a shift in beliefs and opinions w.r.t. extra-marital conduct is definately extant in the data. From the early 70's until about 2002, the trend was towards increasingly conservative thought and opinion, perhaps a natural retraction or readjustment following the sexual revolution of the 60's and 70's. However, 2002 and 2012 mark inflection points of shifting opinions and attitudes regarding extra-marital conduct. From 2012-2016 the proportion of non-traditional opinion increased by `r round(post2012$analysis$PctChange[1], 0)`%. The most significant increase was among women where the proportion of non-traditional opinion grew  `r round(post2012$analysis$PctChange[3], 0)`%  from `r round(post2012$analysis$pMin[3], 0)` in `r round(post2012$analysis$YearLow[3], 0)` to `r round(post2012$analysis$pMax[3], 0)` in `r round(post2012$analysis$YearHigh[3], 0)`. This conflates with the coming of age of a new generation characterized by diminished deference for marriage as an institution, increasing acceptance of same-sex marriage, and the (re) emergence of open marriages, consentual non-manogamy relationships and the like. 
...
# Appendix
## Source Code
Given the size of the code base for this report, only the select functions responsible for statistical computation were included in the report.  

### Difference in Proportions
The following function performs the repeated difference in proportions tests for multiple proportions.
```{r a-eda, code=readLines('../R/diffTest.R')[21:182], echo=TRUE, eval=FALSE}
```

### Chi-square Test for Independence
The following function performs the chi-square test of independence and reports the analysis.
```{r a-eda, code=readLines('../R/x2Test.R')[20:80], echo=TRUE, eval=FALSE}
```

### Chi-square Trend Test
The following function performs the chi-square trend test.
```{r a-eda, code=readLines('../R/x2TrendTest.R')[19:83], echo=TRUE, eval=FALSE}
```
